...
Optionally by specifing the path a new distribution, either generated by the `utils/traffic-distribution-gen.html` or an already present distribution on which the model was trained on can be used by specifying its relative path like so:
```bash
sudo ./utils/run.sh topology/star.dot resources/distributions/peek.csv
```

### Live prediction:

_**Note** that the model is trained with a `BIN_SIZE = 5` and a `window_size=25`, changing these parameters in the `train/constants.py` will require you to re-run the preprocessing and training scripts, wich can be easily done as explained beneath._
...
A specific distribution can be choosen like before:
```bash
sudo ./utils/run.sh topology/simple.dot resources/distributions/peek.csv --live
```
...
#### Visualizing inference performance over the test set
...
To rebuild the dataset using new data or different `BIN_SIZE` / `window_size` you have re run the following scripts:
```bash
